...
const translations = {
  fr: {
    ...
    description:
      "Découvrez une sélection de projets récents réalisés en technologie et en branding. Nous créons des solutions modernes qui renforcent l'image, l'impact et la performance des entreprises.",
    cta: "Voir tout",
  ...
  en: {
    ...
    description:
      "Discover a selection of recent projects completed in technology and branding. We create modern solutions that strengthen the image, impact and performance of businesses.",
    cta: "View all",
...
const getProjectLink = (project) => {
  ...
  if (project.type === "technology") {
    return `/technology/${project.slug}`;
  }

  return `/marketing-brand/${project.slug}`;
};

  const allProjects = useMemo(() => {
    const techProjects = technologyProjects.map((project, index) => ({
      id: `tech-${index}`,
      title: project.projectName?.[lang] || project.projectName?.fr || "",
      category: "Tech",
      image:
        project.heroImage ||
        project.result?.mainMedia ||
        project.result?.gallery?.[0]?.image ||
        "",
      slug: project.slug,
      type: "technology",
    }));

    const brandingProjects = marketingBrandProjects.map((project, index) => ({
      id: `branding-${index}`,
      title: project.projectName?.[lang] || project.projectName?.fr || "",
      category: "Branding",
      image:
    ...
    return [...techProjects, ...brandingProjects];
  }, [lang]);
  ...
  return (
    <section ref={sectionRef} className="section-shell relative overflow-hidden">
      ...
      <div className="page-container">
        <div
          className={`rp-title-entry mx-auto max-w-3xl text-center ${
            visible ? "show" : ""
          }`}
        >
          <h2
        ...
        <div
          className={`rp-title-entry mt-8 flex justify-center ${
            visible ? "show" : ""
          }`}
      ...
      <div className="relative left-1/2 right-1/2 mt-10 w-screen -translate-x-1/2 rp-stage px-4 py-14 sm:px-8 lg:py-20">
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 50% 60% at 50% 50%, rgba(31,108,140,0.10) 0%, transparent 70%)",
          }}
        />

        <div
          className={`rp-carousel-wrap relative z-10 ${
        ...
        >
            ...
            {filteredProjects.map((project, index) => {
              ...
              return (
                <Link
                  ...
                  state={{
                    portfolioTab:
                      project.type === "technology" ? "technology" : "marketing",
                  }}
                  onClick={(e) => {
                    if (!isCenter) {
                      e.preventDefault();
          ...
          <div className="md:hidden">
            <Link
              to={getProjectLink(filteredProjects[currentIndex])}
              state={{
                portfolioTab:
                  filteredProjects[currentIndex]?.type === "technology"
                    ? "technology"
                    : "marketing",
              }}
              className="block"
            >
              <div
                className="rp-card-center mx-auto"
